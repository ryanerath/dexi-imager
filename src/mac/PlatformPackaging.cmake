...
if(BUILD_CLI_ONLY)
    ...
    set(APP_NAME "DEXI Imager")

    # Set all required bundle properties
    set(MACOSX_BUNDLE_BUNDLE_NAME "${APP_NAME}")
    set(MACOSX_BUNDLE_EXECUTABLE_NAME "${PROJECT_NAME}")
    set(MACOSX_BUNDLE_GUI_IDENTIFIER "io.droneblocks.dexi-imager")
    set(MACOSX_BUNDLE_ICON_FILE "AppIcon")
    string(TIMESTAMP CURRENT_YEAR "%Y")
    set(MACOSX_BUNDLE_COPYRIGHT "Copyright © ${CURRENT_YEAR} DroneBlocks. Based on Raspberry Pi Imager © Raspberry Pi Ltd, Apache-2.0.")
...
# Install app icon. The bundle's CFBundleIconFile is "AppIcon", so macOS
# looks for AppIcon.icns / AppIcon.car / Assets.car (in that order; the .car
# wins when present). We always install our .icns; if a .car is provided, it
# overrides for Liquid Glass appearance variants.
...
if(NOT BUILD_CLI_ONLY AND EXISTS "${PRECOMPILED_ICNS}")
    message(STATUS "Installing app icon from ${PRECOMPILED_ICNS}")

    # Always-on cleanup: drop any stale Assets.car / AppIcon.car / .icon
    # that macdeployqt or earlier builds left behind. Without this, the OS
    # would still show the previous artwork even after we replaced .icns.
    add_custom_command(TARGET ${PROJECT_NAME} POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E remove -f "${APP_BUNDLE_PATH}/Contents/Resources/Assets.car"
        COMMAND ${CMAKE_COMMAND} -E remove -f "${APP_BUNDLE_PATH}/Contents/Resources/AppIcon.car"
        ...
        COMMENT "Installing AppIcon.icns and clearing stale icon assets"
    )

    if(EXISTS "${PRECOMPILED_ASSETS_CAR}")
        message(STATUS "Also installing pre-compiled Assets.car for Liquid Glass support")
        add_custom_command(TARGET ${PROJECT_NAME} POST_BUILD
            COMMAND ${CMAKE_COMMAND} -E copy
                "${PRECOMPILED_ASSETS_CAR}"
                "${APP_BUNDLE_PATH}/Contents/Resources/Assets.car"
            COMMENT "Installing Assets.car for Liquid Glass appearance variants"
        )
    endif()
endif()
